...
- mmc:     clean all of the modules in the current directory or supplied directory.
- mim:     Only make all images from product out target directory.
...
function mim()
{
    T=$(gettop)
    if [ "$T" ]; then
        ONE_SHOT_MAKEFILE=true make -C $T -f build/core/main.mk only_images
    else
        echo "Couldn't locate the top of the tree.  Try setting TOP."
    fi
}
...
function mmc()
{
    ...
    if [ -f build/core/envsetup.mk -a -f Makefile -a ! "$S" ]; then
        make clean
    else
	if [ ! "$S" ]; then
	    S="."
	elif [ ! -d "$S" ]; then
	    echo No such supplied directory.
	    return
	fi

        # Find the closest CleanSpec.mk file.
        T=$(gettop)
        local C=$(findmakeclean $S)
        # Remove the path to top as the makefilepath needs to be relative
        local C=`echo $C|sed 's:'$T'/::'`
        if [ ! "$T" ]; then
            echo "Couldn't locate the top of the tree.  Try setting TOP."
        elif [ ! "$C" ]; then
    	    C=$($T/build/tools/findleaves.py --prune=out --prune=.repo --prune=.git $S CleanSpec.mk)
	    if [ ${S:${#S}-1:1} != "/" ]; then
	    	S=$S"/"
	    fi
	    C=${C//$S/ ${PWD#*$T/}\/$S}
...
#	    C=${C//\/\//\/}
	    if [ ! "$C" ]; then
            	echo "Couldn't locate a makeclean from the current directory."
	    else
            	ONE_SHOT_MAKECLEAN=$C make -C $T -f build/core/main.mk modules_clean
	    fi
        else
            ONE_SHOT_MAKECLEAN=$C make -C $T -f build/core/main.mk modules_clean
        fi
...
for f in `/bin/ls vendor/*/vendorsetup.sh vendor/*/*/vendorsetup.sh device/*/*/vendorsetup.sh sc_imx6/scripts/*/*/vendorsetup.sh 2> /dev/null`
do
    echo "including $f"
    . $f
done
